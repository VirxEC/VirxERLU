...
class ground_shot:
    ...
    def run(self, agent: VirxERLU):
        ...
        elif T <= 0 or (T > 0.3 and true_distance_remaining > agent.me.hitbox.length / 2 and not virxrlcu.ground_shot_is_viable(T, agent.boost_accel, agent.me.get_raw(agent), self.offset_target.z, tuple((final_target - agent.me.location).normalize()), distance_remaining)):
